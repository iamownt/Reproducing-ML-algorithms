...
class C4_5(object):
    ...
    def InfoGainRatio(self, X, y, split_attri):
        ...
        for i in range(len(vals)):
            ...
            fea_prob = len(ind)/len(y)
            fea_entropy += -fea_prob*np.log2(fea_prob)
            cond_entropy += prob*self.entropy(y[ind])
        return (self.entropy(y) - cond_entropy)/fea_entropy
    ...
    def fit(self, X, y):
        ...
        def tree_re(X, y_ori, features, y, parent_node=None):
            if len(np.unique(y)) <= 1:
                ...
                item_values = [self.InfoGainRatio(X, y, feature) for feature in features]
